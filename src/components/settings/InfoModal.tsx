...
export function InfoModal({ isOpen, isDark, title, children, actionLabel, actionColor = 'emerald', onClose, onAction, t }: InfoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className={`relative w-full max-w-sm rounded-3xl shadow-2xl p-6 border ${isDark ? 'bg-[var(--bg-tertiary)] border-[var(--border-color)]' : 'bg-white border-[var(--border-color)]'}`}>
        <h3 className={`text-lg font-bold mb-3 ${isDark ? 'text-[var(--text-primary)]' : 'text-slate-800'}`}>{title}</h3>
