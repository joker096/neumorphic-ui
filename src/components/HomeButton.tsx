...
export const HomeButton = ({ isDark, onHome, t }: HomeButtonProps) => {
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center justify-center group pointer-events-auto z-50">
      ...
      <span className={`text-[9px] uppercase tracking-widest mt-2 font-bold ${isDark ? "text-gray-500 group-hover:text-orange-400" : "text-slate-400 group-hover:text-orange-600"} transition-colors`}>{t('chat.hub')}</span>
    </div>
  );
};
