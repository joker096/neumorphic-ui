    ...
    >
       ...
       >
          <div className="flex items-center justify-between mb-2">
             <h3 className="text-xl font-bold tracking-tight">Создать канал</h3>
          ...
          <div className="flex flex-col gap-2">
             <label className={`text-xs pl-2 font-semibold tracking-wide uppercase ${isDark ? "text-gray-400" : "text-slate-500"}`}>Имя канала</label>
             <input autoFocus value={name} onChange={e => setName(e.target.value)} type="text" className={`w-full h-12 rounded-2xl px-4 outline-none transition-all ${isDark ? "bg-[#13151b] border border-white/5 focus:border-orange-500/50" : "bg-slate-50 border border-black/5 focus:border-orange-500/50"}`} placeholder="Название..." />
          </div>

          <div className="flex flex-col gap-2">
             <label className={`text-xs pl-2 font-semibold tracking-wide uppercase ${isDark ? "text-gray-400" : "text-slate-500"}`}>Описание (опционально)</label>
             <input value={desc} onChange={e => setDesc(e.target.value)} type="text" className={`w-full h-12 rounded-2xl px-4 outline-none transition-all ${isDark ? "bg-[#13151b] border border-white/5 focus:border-orange-500/50" : "bg-slate-50 border border-black/5 focus:border-orange-500/50"}`} placeholder="О чем ваш канал?" />
          </div>
