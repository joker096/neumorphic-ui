...
export const ChatInputOverlay = ({
  ...
  return (
    <div className={`absolute bottom-0 left-0 right-0 md:static md:bottom-auto md:left-auto md:right-auto md:rounded-b-[48px] md:border-none md:bg-transparent md:shadow-none md:z-0 z-60 ${isDark ? "bg-[#1a1d24]/95 border-t border-white/10 md:border-white/5 md:backdrop-blur-xl" : "bg-white/95 border-t border-black/10 md:border-black/5 md:backdrop-blur-xl md:shadow-inner"}`} style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      ...
      <AnimatePresence>
        {showSchedulePopup && !activeChat.isChannel && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className={`w-full p-4 rounded-xl flex flex-col gap-3 ${isDark ? "bg-[#13151b] border border-white/5" : "bg-white border border-black/5 shadow-sm"}`}
          >
      ...
      {activeChat.isChannel ? (
        <div className={`w-full py-2.5 rounded-xl flex items-center justify-center cursor-pointer transition-colors font-medium text-sm tracking-wide ${isDark ? "bg-[#13151b] hover:bg-[#20242e] text-orange-400 border border-white/5" : "bg-white hover:bg-slate-50 text-orange-600 border border-black/5 shadow-sm"}`} onClick={() => {
        ...
        <div className="flex items-center gap-3 w-full">
          <div title={t('chat.attachFile')} className="relative group">
            ...
            />
            <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all flex-shrink-0 relative z-0 ${isDark ? "bg-[#13151b] text-gray-400 group-hover:text-white group-hover:bg-white/5" : "bg-[#f4f7f9] text-slate-500 group-hover:text-slate-800 group-hover:bg-slate-200"}`}><Plus size={18} /></div>
          </div>
          ...
          {replyTarget && (
            <div className={`mt-1 px-3 py-2 rounded-xl border-l-2 flex items-start justify-between gap-2 ${isDark ? "bg-[#1a1d24]/80 border-orange-400/60 text-gray-300" : "bg-white/80 border-orange-500 text-slate-700"}`}>
