        ...
        {chat.online && (
          <div
            className={`absolute -bottom-0.5 -right-0.5 w-[10px] h-[10px] rounded-full border-[2px] ${
              isDark ? "bg-green-400 border-[var(--bg-tertiary)]" : "bg-emerald-500 border-[var(--bg-primary)]"
            }`}
          />
        )}
      </div>
      ...
      <div className="flex-1 flex items-center gap-1.5 sm:gap-2 min-w-0">
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <span
              className={`w-1.5 h-1.5 rounded-full shrink-0 ${chat.online ? "bg-emerald-400" : "bg-gray-500"}`}
            />
            <span
              className={`text-[8px] sm:text-[9px] font-semibold tracking-wider uppercase shrink-0 ${isDark ? "text-orange-400/90" : "text-orange-500/90"}`}
            >
          ...
          {typing && (
            <div className="flex items-center gap-1 mt-0.5">
              <span
                className={`text-[11px] italic animate-pulse ${isDark ? "text-emerald-400" : "text-emerald-600"}`}
              >
                {t("chat.typing")}
              </span>
              <span className="flex gap-0.5">
                <span
                  className={`w-1 h-1 rounded-full animate-bounce ${isDark ? "bg-emerald-400" : "bg-emerald-600"}`}
