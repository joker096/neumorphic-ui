      ...
      <div className="flex items-center gap-2 sm:gap-3 px-1 sm:px-2 overflow-x-auto pb-2 scrollbar-none shrink-0" onWheel={(e) => { e.currentTarget.scrollLeft += e.deltaY; }}>
        ...
        >
          <div className={`relative w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-transform duration-200 active:scale-95 ${isDark ? "bg-[#1f222a] border border-[var(--border-color)] border-dashed" : "bg-[var(--bg-primary)] border border-[var(--border-color)] border-dashed"}`}>
            ...
            <span className={`text-[8px] sm:text-[9px] font-semibold tracking-wide transition-colors ${isDark ? "text-gray-300 group-hover:text-gray-100" : "text-slate-500 group-hover:text-slate-800"}`}>
                {t("header.myStory")}
              </span>
        </div>
