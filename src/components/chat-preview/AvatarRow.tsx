    ...
    <div className="flex flex-col w-full overflow-visible mb-2 pt-2 pb-1 bg-transparent shrink-0">
      ...
      <div className="flex items-center gap-3 sm:gap-4 px-2 sm:px-3 overflow-x-auto pb-2 scrollbar-none shrink-0" onWheel={(e) => { e.currentTarget.scrollLeft += e.deltaY; }}>
        ...
        {ONLINE_CONTACTS.map((c) => (
          ...
          >
            <div className={`relative w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-transform duration-300 group-hover:scale-110 active:scale-95 ${isDark ? "bg-[#1a1d24] shadow-[0_2px_8px_rgba(0,0,0,0.3)] border border-white/5" : "bg-[#eaeff4] shadow-[4px_4px_8px_rgba(165,175,190,0.3),_-4px_-4px_8px_rgba(255,255,255,0.8),_inset_1.5px_1.5px_2px_rgba(255,255,255,1)] border border-black/5"}`}>
              <div className="w-[85%] h-[85%] rounded-full overflow-hidden p-[2px]">
                <div className={`w-full h-full rounded-full bg-gradient-to-br ${c.color} flex items-center justify-center text-white font-bold text-lg`}>
                  {c.name.charAt(0)}
                </div>
              </div>
            </div>
