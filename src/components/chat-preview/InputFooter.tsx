...
const InputFooterImpl = ({ isDark = false, isChannel, isMuted, placeholder, t, onMuteToggle }: InputFooterProps) => {
  return (
    <div
      className={`p-4 md:p-5 flex items-center justify-center gap-3 md:gap-4 relative z-10 ${isDark ? "bg-[var(--bg-tertiary)]/90 border-t border-[var(--border-color)] backdrop-blur-md" : "bg-[var(--bg-primary)]/90 border-t border-[var(--border-color)] backdrop-blur-md"}`}
    >
      {isChannel ? (
        <button
          type="button"
...
export const InputFooter = React.memo(InputFooterImpl);
