      ...
      <div className={`px-3 sm:px-5 pt-3 sm:pt-4 pb-2 flex flex-col gap-2 overflow-x-auto scrollbar-none ${isDark ? "bg-[#1a1d24]/60" : "bg-[#f4f7f9]/60"}`} onWheel={(e) => { e.currentTarget.scrollLeft += e.deltaY; }}>
        <div className="flex items-center gap-2">
          <button
            ...
            className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-[10px] font-bold whitespace-nowrap transition-colors ${showFilterMenu ? "bg-orange-500 text-white" : isDark ? "bg-white/5 text-gray-400" : "bg-black/5 text-slate-500"}`}
          >
            <ListFilter size={14} />
          </button>
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              {['', 'me', 'them'].map((v) => (
                <button key={v} onClick={() => setFilterBySender(v)}
                  className={`px-2 py-0.5 rounded-full text-[10px] ${filterBySender === v ? "bg-green-500 text-white" : isDark ? "bg-white/5 text-gray-400" : "bg-black/5 text-slate-500"}`}
                >
        ...
        <div className="flex items-center gap-2">
          {['all', 'photos', 'audio', 'links'].map((tab) => (
            <button key={tab} onClick={() => setMediaTab(tab)}
              className={`px-2 py-1 sm:px-3 sm:py-1.5 rounded-full text-[9px] sm:text-[11px] font-bold whitespace-nowrap transition-colors ${mediaTab === tab ? "bg-orange-500 text-white shadow-md" : isDark ? "bg-white/5 text-gray-400 hover:text-white" : "bg-black/5 text-slate-500 hover:text-slate-800"}`}
            >
