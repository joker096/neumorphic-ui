...
export const Tooltip = ({ children, content, position = 'top', theme }: { children: React.ReactNode; content: string; position?: 'top' | 'bottom' | 'left' | 'right'; theme?: string }) => {
  ...
  return (
    <div ref={ref} onMouseEnter={showTooltip} onMouseLeave={hideTooltip} onFocus={showTooltip} onBlur={hideTooltip} tabIndex={0} style={containerStyle}>
      {children}
      {visible && (
        <div style={tooltipStyle} className="px-2 py-1 rounded-md text-xs font-medium whitespace-nowrap transition-opacity shadow-lg bg-[var(--bg-secondary)] text-[var(--text-primary)]">
          {content}
        </div>
      )}
    </div>
